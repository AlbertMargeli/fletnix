...
$.getJSON ("llista_original.php", function(pelis) {
    ...
    for (n=0; n<pelis.length; n++) { 
        ...
        $('.splide__list').append('<li class="splide__slide movie"><div class="movie-back"><h2 class="titol">'+pelis[n]["titol"]+'</h2><img class="portada" src="img/covers/'+pelis[n]["image"]+'.jpg"><h3 class="director">'+pelis[n]["director"]+'</h3><p class="dades"><span class="any">'+pelis[n]["any"]+'</span> / <span class="pais">'+pelis[n]["pais"]+'</span></p><a class="google" href="https://www.google.com/search?q='+pelis[n]["titol"]+' '+pelis[n]["director"]+' " target="_blank">Més info</a><div class="stars">'+mostrarEstrellas+'</div><div></li>');

        
    }

    var splide = new Splide( '.splide' , {type: 'loop', perPage: 6} );
    splide.mount();
